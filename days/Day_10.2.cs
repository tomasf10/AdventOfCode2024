...
public static class Day_10_2
{
        ...
        foreach (var basee in bases)
        {
            ...
            foreach (var top in tops)
            {
                if (Math.Abs(basee.Item1 - top.Item1) < 10 && Math.Abs(basee.Item2 - top.Item2) < 10)
                {
                    ...
                    var raiting = IsRechable(basee, top, map);
    ...
    private static int IsRechable((int, int) currentNode, (int, int) top, List<List<int>> map)
    {

        var result = currentNode.Item1 == top.Item1 && currentNode.Item2 == top.Item2 ? 1 : 0;
        var nextSteps = GetNextNodes(currentNode, map);

        foreach (var step in nextSteps)
        {
            result += IsRechable(step, top, map);
        }
